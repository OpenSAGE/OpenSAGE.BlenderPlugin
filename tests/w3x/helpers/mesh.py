...
def get_mesh():
    ...
    mesh.triangles = [get_triangle([4, 2, 0], get_vec(0.0, 0.0, 1.0), 0.63),
                      get_triangle([2, 7, 3], get_vec(
                          0.0, -1.0, 0.0), 0.63),
                      get_triangle(
                          [6, 5, 7], get_vec(-1.0, 0.0, 0.0), 0.63),
                      get_triangle([1, 7, 5], get_vec(
                          0.0, 0.0, -1.0), 0.63),
                      get_triangle([0, 3, 1], get_vec(1.0, 0.0, 0.0), 0.63),
                      get_triangle([4, 1, 5], get_vec(0.0, 1.0, 0.0), 0.63),
                      get_triangle([4, 6, 2], get_vec(0.0, 0.0, 1.0), 0.63),
                      get_triangle([2, 6, 7], get_vec(
                          0.0, -1.0, 0.0), 0.63),
                      get_triangle(
                          [6, 4, 5], get_vec(-1.0, 0.0, 0.0), 0.63),
                      get_triangle([1, 3, 7], get_vec(
                          0.0, 0.0, -1.0), 0.63),
                      get_triangle([0, 2, 3], get_vec(1.0, 0.0, 0.0), 0.63),
                      get_triangle([4, 0, 1], get_vec(0.0, 1.0, 0.0), 0.63)]

    mesh.tex_coords = [get_vec(0.0, 0.1),
                       get_vec(0.0, 0.4),
                       get_vec(1.0, 0.6),
                       get_vec(0.3, 0.1),
                       get_vec(0.2, 0.2),
...
def get_mesh_minimal():
    return Mesh(
        ...
        bounding_box=get_bounding_box(),
        bounding_sphere=get_bounding_sphere(),
        verts=[get_vec(1.0, 1.0, 1.0)],
        normals=[get_vec(0.577, 0.577, 0.577)],
        tangents=[],
        bitangents=[],
        tex_coords=[get_vec(0.0, 0.1)],
        shade_indices=[1],
        triangles=[get_triangle(
            [4, 2, 0], get_vec(0.0, 0.0, 1.0), 0.63)],
